/**
 * JavaScript file for the TV Shows page.
 * Handles fetching, displaying, and UI interactions for TV content.
 */
...
// Function to update banner slideshow for TV shows
function updateBannerForTVShows() {
    ...
    if (extraInfo.length > 0) {
        ...
        if (subtitleElement) {
            ...
            if (subtitleElement.textContent.includes(latestTag)) {
                ...
                if (badge) {
                    badge.style.backgroundColor = 'rgba(0, 114, 210, 0.85)'; // Disney+ blue for TV content
...
function getRatingColor(rating) {
    ...
    if (rating >= 6) return '#0072d2'; // Disney+ blue for good ratings (TV shows)
...
function fetchTVShows(containerClass, endpoint, usePosterPath = false) {
    console.log(`Fetching TV shows for ${containerClass} with endpoint ${endpoint}`);
    ...
    containers.forEach((container) => {
        fetch(`https://api.themoviedb.org/3/${endpoint}api_key=${api_Key}`)
            ...
            .then(data => {
                const shows = data.results || [];
                console.log(`Got TV show data for ${containerClass}, found ${shows.length} items`);
                ...
                shows.forEach(show => {
                    ...
                    let pathToUse = usePosterPath ? show.poster_path : show.backdrop_path;
                    // Create item element
                    ...
                    itemElement.addEventListener('click', () => {
                        ...
                        window.location.href = `../../movie_details/movie_details.html?media=tv&id=${mediaId}`;
                    });
                    ...
                    titleElement.textContent = show.name || show.title;
...
async function fetchSearchResults(query) {
    try {
        ...
        // Filter for TV Shows Only and ensure each has an image
        return results
...
function displaySearchResults(results) {
    searchResults.innerHTML = '';
    results.map(item => {
        const shortenedTitle = item.name || 'Unknown Title';
        const date = item.first_air_date || '';

        let buttonText = "Add to WatchList"; // Set default button text

        // Check if the show is already in WatchList
        ...
        showItem.innerHTML = `<div class = "search-item-thumbnail">
                                    <img src ="${item.poster_path}">
                                </div>
                                <div class ="search-item-info">
                                    <h3>${shortenedTitle}</h3>
                                    <p>TV Show <span> &nbsp; ${date}</span></p>
                                </div>
                                <button class="watchListBtn" id="${item.id}">${buttonText}</button>`;

        const watchListBtn = showItem.querySelector('.watchListBtn');
        ...
        const thumbnail = showItem.querySelector('.search-item-thumbnail');
        const info = showItem.querySelector('.search-item-info');
        ...
        showItem.setAttribute('class', 'movie-list');
...
// Function to add a show to WatchList
function addToWatchList(show) {
    // Check if the show is not already in the WatchList list
    if (!watchlist.find(watchlistItem => watchlistItem.id === show.id)) {
        watchlist.push(show);
        localStorage.setItem('watchlist', JSON.stringify(watchlist)); // Store in Local Storage
        const watchListBtn = document.querySelector(`[id="${show.id}"]`);
        if (watchListBtn) {
...
document.addEventListener('DOMContentLoaded', () => {
    // Update banner for TV shows section
    updateBannerForTVShows();

    // Fetch TV show data for each section
    fetchTVShows('netflix-container', 'discover/tv?with_networks=213&', true); // Netflix originals with poster_path
    fetchTVShows('disney-container', 'discover/tv?with_networks=2739&sort_by=popularity.desc&');
    fetchTVShows('drama-tv-container', 'discover/tv?with_genres=18&');
    fetchTVShows('crime-tv-container', 'discover/tv?with_genres=80&');
    fetchTVShows('comedy-tv-container', 'discover/tv?with_genres=35&');
    fetchTVShows('actionadventure-container', 'discover/tv?with_genres=10759&sort_by=popularity.desc&');
    fetchTVShows('mystery-container', 'discover/tv?with_genres=9648&sort_by=popularity.desc&');
    fetchTVShows('fantasy-container', 'discover/tv?with_genres=10765&sort_by=popularity.desc&');
    fetchTVShows('reality-container', 'discover/tv?with_genres=10764&sort_by=popularity.desc&');
    fetchTVShows('scifi-tv-container', 'discover/tv?with_genres=10765&');
    fetchTVShows('documentary-tv-container', 'discover/tv?with_genres=99&');

    // Setup scroll functionality for each container
    setupScroll('netflix-container', 'netflix-previous', 'netflix-next');
    setupScroll('disney-container', 'disney-previous', 'disney-next');
    ...
    setupScroll('actionadventure-container', 'actionadventure-previous', 'actionadventure-next');
    setupScroll('mystery-container', 'mystery-previous', 'mystery-next');
    setupScroll('fantasy-container', 'fantasy-previous', 'fantasy-next');
    setupScroll('reality-container', 'reality-previous', 'reality-next');
