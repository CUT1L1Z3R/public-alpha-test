    ...
    if (type === "anime") {
        ...
        switch (server) {
            ...
            case "iframe.pstream.org":
                if (type === "tv") {
                    // For TV shows, default to first episode of first season
                    embedURL = `https://iframe.pstream.org/embed/tmdb-tv-${id}?theme=grape&language=en&logo=false&downloads=false&language-order=en%2Chi%2Cfr%2Cde%2Cnl%2Cpt&allinone=false&scale=1.0&fedapi=false&interface-settings=false&tips=false&has-watchparty=false`;
                } else {
                    embedURL = `https://iframe.pstream.org/embed/tmdb-movie-${id}?theme=grape&language=en&logo=false&downloads=false&language-order=en%2Chi%2Cfr%2Cde%2Cnl%2Cpt&allinone=false&scale=1.0&fedapi=false&interface-settings=false&tips=false&has-watchparty=false`;
                }
                break;
            case "vidsrc.cc":
            ...
            default:
                ...
                if (type === "tv") {
                    embedURL = `https://iframe.pstream.org/embed/tmdb-tv-${id}?theme=grape&language=en&logo=false&downloads=false&language-order=en%2Chi%2Cfr%2Cde%2Cnl%2Cpt&allinone=false&scale=1.0&fedapi=false&interface-settings=false&tips=false&has-watchparty=false&subtitles=true&branding=false&watermark=false`;
                } else {
                    embedURL = `https://iframe.pstream.org/embed/tmdb-movie-${id}?theme=grape&language=en&logo=false&downloads=false&language-order=en%2Chi%2Cfr%2Cde%2Cnl%2Cpt&allinone=false&scale=1.0&fedapi=false&interface-settings=false&tips=false&has-watchparty=false&subtitles=true&branding=false&watermark=false`;
                }
                break;
        }
    }
...
function playEpisode(tvId, seasonNumber, episodeNumber) {
    ...
    switch (server) {
        ...
        case "vidlink.pro":
            ...
            break;
        case "iframe.pstream.org":
            embedURL = `https://iframe.pstream.org/embed/tmdb-tv-${tvId}?season=${seasonNumber}&episode=${episodeNumber}&theme=grape&language=en&logo=false&downloads=false&language-order=en%2Chi%2Cfr%2Cde%2Cnl%2Cpt&allinone=false&scale=1.0&fedapi=false&interface-settings=false&tips=false&has-watchparty=false`;
            break;
        case "vidsrc.cc":
