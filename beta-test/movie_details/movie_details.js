    ...
    if (type === "anime") {
        ...
        switch (server) {
            ...
            case "iframe.pstream.org":
                if (type === "tv") {
                    // For TV shows, default to first episode of first season
                    embedURL = `https://iframe.pstream.org/embed/tmdb-tv-${id}?theme=violet&language=en&logo=false&downloads=false&language-order=en%2Chi%2Cfr%2Cde%2Cnl%2Cpt&allinone=false&scale=1.0&fedapi=false&interface-settings=false&tips=false&has-watchparty=false&subtitles=true`;
                } else {
                    embedURL = `https://iframe.pstream.org/embed/tmdb-movie-${id}?theme=violet&language=en&logo=false&downloads=false&language-order=en%2Chi%2Cfr%2Cde%2Cnl%2Cpt&allinone=false&scale=1.0&fedapi=false&interface-settings=false&tips=false&has-watchparty=false&subtitles=true`;
                }
                break;
            case "vidsrc.cc":
...
function playEpisode(tvId, seasonNumber, episodeNumber) {
    ...
    switch (server) {
        ...
        case "vidlink.pro":
            ...
            break;
        case "iframe.pstream.org":
            embedURL = `https://iframe.pstream.org/embed/tmdb-tv-${tvId}?season=${seasonNumber}&episode=${episodeNumber}&theme=violet&language=en&logo=false&downloads=false&language-order=en%2Chi%2Cfr%2Cde%2Cnl%2Cpt&allinone=false&scale=1.0&fedapi=false&interface-settings=false&tips=false&has-watchparty=false&subtitles=true`;
            break;
        case "vidsrc.cc":
