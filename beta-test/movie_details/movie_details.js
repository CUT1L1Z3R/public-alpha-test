    ...
    if (type === "anime") {
        ...
        switch (server) {
            ...
            case "iframe.pstream.org":
                if (type === "tv") {
                    embedURL = `https://iframe.pstream.org/embed/tmdb-tv-${id}/1/1?theme=red&language=en&logo=false&downloads=false&language-order=en%2Chi%2Cfr%2Cde%2Cnl%2Cpt&allinone=true&scale=1.0&backlink=https%3A%2F%2Fandoks.cc&fedapi=false&interface-settings=false&tips=false&has-watchparty=false`;
                } else {
                    embedURL = `https://iframe.pstream.org/embed/tmdb-movie-${id}?theme=red&language=en&logo=false&downloads=false&language-order=en%2Chi%2Cfr%2Cde%2Cnl%2Cpt&allinone=true&scale=1.0&backlink=https%3A%2F%2Fandoks.cc&fedapi=false&interface-settings=false&tips=false&has-watchparty=false`;
                }
                break;
            case "vidsrc.cc":
...
function playEpisode(tvId, seasonNumber, episodeNumber) {
    ...
    switch (server) {
        ...
        case "vidlink.pro":
            ...
            break;
        case "iframe.pstream.org":
            embedURL = `https://iframe.pstream.org/embed/tmdb-tv-${tvId}/${seasonNumber}/${episodeNumber}?theme=red&language=en&logo=false&downloads=false&language-order=en%2Chi%2Cfr%2Cde%2Cnl%2Cpt&allinone=true&scale=1.0&backlink=https%3A%2F%2Fandoks.cc&fedapi=false&interface-settings=false&tips=false&has-watchparty=false`;
            break;
        case "vidsrc.cc":
